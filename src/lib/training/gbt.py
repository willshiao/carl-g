...
def perform_gbt_training(
    data, output_dir, device, input_size: int, has_features: bool, enc_zoo, timing_run=False, return_memory=False
):
    ...
    for epoch in tqdm(range(1, FLAGS.epochs + 1)):
        ...
        memory_used = full_train(epoch - 1)
        if timing_run:
            torch.cuda.synchronize()
    ...
    if return_memory:
        return encoder, representations, time_bundle, memory_list
